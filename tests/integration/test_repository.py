...
def insert_order_line(sqlite_session) -> str:  # создание товарной позиции
    sqlite_session.execute(
        text('INSERT INTO order_lines (orderid, sku, qty)'
             ' VALUES ("order1", "GENERIC-SOFA", 12)')
    )

    [[orderline_id]] = sqlite_session.execute(
        text('SELECT id FROM order_lines WHERE orderid=:orderid AND sku=:sku').bindparams(orderid="order1", sku="GENERIC-SOFA"),
    )

    return orderline_id

def insert_batch(sqlite_session, batch_id) -> str:     # создание партии товара
    sqlite_session.execute(
...
def insert_allocation(sqlite_session, orderline_id, batch_id) -> None:     # размещение заказа в партии
    sqlite_session.execute(
...
def get_allocations(sqlite_session, batch_id) -> str:
    rows = list(
        sqlite_session.execute(
...
def test_repository_can_save_a_batch(sqlite_session):
    """
    ...
    rows = list(sqlite_session.execute(text('SELECT reference, sku, _purchased_quantity, eta FROM "batches"')))

    assert rows == [("batch1", "RUSTY-SOAPDISH", 100, None)]



def test_repository_can_retrieve_a_batch_with_allocations(sqlite_session):
    """
...
def test_updating_a_batch(sqlite_session):
    """
